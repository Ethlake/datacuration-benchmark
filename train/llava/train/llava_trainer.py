...
class LLaVATrainer(Trainer):
    ...
    #     # 调用父类：这里会做 forward + backward，把梯度写进 param.grad
    ...
    #                     print(f"[train_step] step={self.state.global_step} {name} grad=None")
    #                 else:
    #                     gnorm = p.grad.detach().float().norm().item()
    #                     print(f"[train_step] step={self.state.global_step} {name} grad_norm={gnorm:.6f}")
    #                 break  # 找到第一个 projector 参数就打印，避免太吵
